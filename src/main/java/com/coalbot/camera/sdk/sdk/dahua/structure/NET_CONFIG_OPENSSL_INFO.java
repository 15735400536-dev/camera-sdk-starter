 ...
 * @date 2021/3/2
 */
public class NET_CONFIG_OPENSSL_INFO extends NetSDKLibStructure.SdkStructure {
    public byte[]           szSsleay = new byte[NetSDKLibStructure.MAX_PATH];        // openssl协议库
  // windows下 为ssleay32.dll库全路径，当路径为空时默认从当前NetSDK.dll路径下查找加载
  // linux下	为libssl.so	  库全路径，当路径为空时默认从系统路径下查找加载
    public byte[]           szLibeay = new byte[NetSDKLibStructure.MAX_PATH];        // openssl加密库
