...
import com.coalbot.camera.sdk.sdk.dahua.NetSDKLib.NET_IN_LOGIN_WITH_HIGHLEVEL_SECURITY;
import com.coalbot.camera.sdk.sdk.dahua.NetSDKLib.NET_OUT_LOGIN_WITH_HIGHLEVEL_SECURITY;
import com.coalbot.camera.sdk.sdk.dahua.NetSDKLib.fServiceCallBack;
...
public class AutoRegisterModule {
	...
	// 设备信息
	public static NetSDKLibStructure.NET_DEVICEINFO_Ex m_stDeviceInfo = new NetSDKLibStructure.NET_DEVICEINFO_Ex();
	...
	public static boolean snapPicture(LLong m_hLoginHandle, int chn) {
		// 发送抓图命令给前端设备，抓图的信息
		NetSDKLibStructure.SNAP_PARAMS msg = new NetSDKLibStructure.SNAP_PARAMS();
	 ...
	 * @return
	 */
	public static NetSDKLibStructure.CFG_DVRIP_INFO getDVRIPConfig(LLong m_hLoginHandle) {
		NetSDKLibStructure.CFG_DVRIP_INFO msg = new NetSDKLibStructure.CFG_DVRIP_INFO();
		
		if(!ToolKits.GetDevConfig(m_hLoginHandle, -1, NetSDKLibStructure.CFG_CMD_DVRIP, msg)) {
			return null;
		}
	 ...
	 * @return
	 */
	public static boolean setDVRIPConfig(LLong m_hLoginHandle, boolean enable, String address, int nPort, byte[] deviceId, NetSDKLibStructure.CFG_DVRIP_INFO info) {
		NetSDKLibStructure.CFG_DVRIP_INFO msg = info;
		// 主动注册配置个数
		msg.nRegistersNum = 1;  
		...
		return ToolKits.SetDevConfig(m_hLoginHandle, -1, NetSDKLibStructure.CFG_CMD_DVRIP, msg);
	}
	...
	public static boolean startTalk(LLong m_hLoginHandle) {
		...
		talkEncode.write();
		if(LoginModule.netsdk.CLIENT_SetDeviceMode(m_hLoginHandle, NetSDKLibStructure.EM_USEDEV_MODE.NET_TALK_ENCODE_TYPE, talkEncode.getPointer())) {
        ...
        if (LoginModule.netsdk.CLIENT_SetDeviceMode(m_hLoginHandle, NetSDKLibStructure.EM_USEDEV_MODE.NET_TALK_SPEAK_PARAM, speak.getPointer())) {
		...
		talkTransfer.write();
		if(LoginModule.netsdk.CLIENT_SetDeviceMode(m_hLoginHandle, NetSDKLibStructure.EM_USEDEV_MODE.NET_TALK_TRANSFER_MODE, talkTransfer.getPointer())) {
