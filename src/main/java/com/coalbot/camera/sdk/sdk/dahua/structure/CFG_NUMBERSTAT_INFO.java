 ...
 */
public class CFG_NUMBERSTAT_INFO extends NetSDKLibStructure.SdkStructure {
	/**
	 * 规则名称,不同规则不能重名
	 */
	 ...
	 * {@link com.coalbot.camera.sdk.sdk.dahua.NetSDKLibStructure.CFG_LINK_TYPE}
	 */
    public int              emType;
	/**
	 * 正常运动方向, 按照此方向行驶是正向，反过来是逆行 只对出入口类型有效。第一个点是起点，第二个点是终点，点的坐标归一化到[0,8191]区间。
	 */
    public NetSDKLibStructure.CFG_POLYGON[] stDirection = new NetSDKLibStructure.CFG_POLYGON[2];
	/**
	 * 记录周期，单位：分钟，1~10
	 */
    ...
    */
	...
	public CFG_NUMBERSTAT_INFO() {
		...
		for (int i = 0; i < stuTimeSection.length; i++) {
			stuTimeSection[i] = new NetSDKLibStructure.TIME_SECTION_WEEK_DAY_10();
		}
		for (int i = 0; i < stDirection.length; i++) {
			stDirection[i] = new NetSDKLibStructure.CFG_POLYGON();
		}
		...
		for (int i = 0; i < stuRemoteTimeSection.length; i++) {
			stuRemoteTimeSection[i] = new NetSDKLibStructure.TIME_SECTION_WEEK_DAY_10();
		}
	}
}
