package com.coalbot.camera.sdk.sdk.uniview;
...
public interface NetDEVSDKLib extends Library {
	NetDEVSDKLib NETDEVSDK_INSTANCE = (NetDEVSDKLib)Native.loadLibrary(CameraSdkUtils.getSdkPath(CameraBrand.Uniview), NetDEVSDKLib.class);
	...
	*/
	...
	/**
	 * @struct tagNETDEVComAttachInfo
	 * @brief 目标证据链关联图片详细信息
	 * @attention 无 None
	 */
	public class NETDEV_COM_ATTACH_INFO_S extends Structure
	{
	    public int   	udwAttachIndex;    /* 目标证据链对应的图片在图像列表中的索引，从1开始，0为无效值。 */
	    public int   	udwComIndex;       /* 目标证据链对应的图片在证据链中的顺序，从1开始，0位无效值,过车代表过车第一张、第二张，违法代表违法顺序即违法前，违法中，违法后。 */
	    public byte[]   byRes = new byte[16];                                                        /* 保留字段 */
	}
	...
	public class NETDEV_PERSON_STRUCT_INFO_S extends Structure
	{
		...
		public Pointer  pstRuleInfo;                              /* 规则信息 需动态申请内存   NETDEV_RULE_INFO_S */
		public int                     udwConfidence;                                 /* 检测置信度, 取值范围:[0-100] */
		public int                     udwComAttachNum;                               /* 目标证据链关联图片数量 */
		public Pointer pstComAttachList;                              /* 目标证据链关联图片详细信息,目标证据链关联图片数量不为0时有效, 需动态申请内存 */
		public byte[]   byRes = new byte[112];                                     /* 保留字段 */
	}
	...
	public class NETDEV_NON_MOTOR_VEH_INFO_S extends Structure
	{
		...
		public Pointer  pstRuleInfo;                              /* 规则信息 需动态申请内存   NETDEV_RULE_INFO_S */
		public Pointer       pstLaneInfo;                                   /* 车道信息 需动态申请内存*/
		public int                     udwConfidence;                                 /* 检测置信度, 取值范围:[0-100] */
		public int                     udwDirectionType;                              /* 运动方向，未检测时可选。 1：西向东（简称东，下同）； 2：东向西（西）； 3：北向南（南）； 4：南向北（北）； 5：西南到东北（东北）； 6：东北到西南（西南）； 7：西北到东南（东南）； 8：东南到西北（西北）； 9：其他 */
		public int                     udwComAttachNum;                               /* 目标证据链关联图片数量 */
		public Pointer pstComAttachList;                              /* 目标证据链关联图片详细信息,目标证据链关联图片数量不为0时有效, 需动态申请内存 */
		public int udwPlatePicAttachIndex;						  /* 车牌对应的小图在图像列表中的索引 */
		public Pointer pstPlateAttr;							  /* 车牌属性信息 */
		public byte[]   byRes = new byte[96];                                    /* 保留字段 */
	}
	...
	public class NETDEV_VEH_INFO_S extends Structure
	{
		...
		public Pointer 	pstLaneInfo;                                   /* 车道信息 需动态申请内存*/
		public int               udwConfidence;                                 /* 检测置信度, 取值范围:[0-100] */
		public int               udwTriggerType;                                /* 抓拍触发类型，未检测时可选。0-相机触发的抓拍（包括手动抓拍和自动抓拍），1-平台下发的抓拍，抓拍类型是手动抓拍，2-平台下发的抓拍，抓拍类型是联动抓拍，3-相机手动抓拍，通过SDK触发相机抓拍，4-相机视频检测抓拍， 5-相机外设抓拍, 注：如果抓拍设备能够识别触发抓拍的类型，尽量填写准确的值。默认填写0。 */
		public int               udwDirectionType;                              /* 运动方向，未检测时可选。 1：西向东（简称东，下同）； 2：东向西（西）； 3：北向南（南）； 4：南向北（北）； 5：西南到东北（东北）； 6：东北到西南（西南）； 7：西北到东南（东南）； 8：东南到西北（西北）； 9：其他 */
		public int               udwComAttachNum;                               /* 目标证据链关联图片数量 */
		public Pointer 			 pstComAttachList;                        		/* 目标证据链关联图片详细信息,目标证据链关联图片数量不为0时有效, 需动态申请内存 */
		public byte[]   byRes = new byte[100];                                    /* 保留字段 */
	}
	...
	*/
	...
	public class NETDEV_ACS_ATTENDANCE_LOG_INFO_S extends Structure
	{
		...
		public int udwGender;                               /* 人员性别 0:未知 1:男性 2:女性 识别终端+身份证读卡器上报 */
		public byte[]   szIdentityNo = new byte[NETDEV_LEN_64];             /* 身份证号 字符串长度范围[1, 64] 识别终端+身份证读卡器上报。
	 */
		public byte[]    byRes = new byte[188];                      /* 保留字段 */
	}
     ...
     /**
       ...
       */
	    ...
	    */
	    ...
	    /**
	    * @struct tagNETDEVManualSnapParam
	    * @brief 手动抓拍参数
	    * @attention 无 None
	    */
	    public class NETDEV_MANUAL_SNAPSHOT_PARAM_S extends Structure
	    {
	    	public int                         dwLaneCode;                      /* 抓拍车道号, 范围:[1,254] */
	    	public int                         dwTriggerID;                     /* 抓拍触发ID, 范围:[1,65535] */
	    	public byte[]    byRes             = new byte[32];                  /* 保留字段 */
	    }
	...
	* @param [IN] lpUserID                用户登录ID
	* @param [IN] pstResChanegeEventData  资源变更事件信息对应结构体的长度 Length of structure for resources change event information
	* @param [IN] lpUserData              用户数据 User data
	* @note
	*/
	 ...
	 * @param [IN]  cbPlayDataCallBack   码流数据回调函数指针NETDEV_SOURCE_DATA_CALLBACK_PF
	 * @param [IN]  lpUserData           用户数据 User data
    ...
    * 接收火点告警消息的回调函数          Callback function to receive alarm information
    * @param [IN] lpUserID                用户登录句柄 User login ID
    * @param [IN] pstAlarmInfo            火点告警信息 Conflagration Alarm information
    * @param [IN] lpUserData              用户数据 User data
    * @note
    */
    ...
    * @param [IN] lpUserID                用户登录句柄 User login ID
    * @param [IN] pstAlarmPicData         吸烟告警信息 Alarm information
    * @param [IN] lpUserData              用户数据 User data
    * @note
    */
    ...
    * 接收温度检测告警消息的回调函数
    * @param [IN] lpUserID                            用户登录句柄
    * @param [IN] pstTemperatureDetectionInfo         温度检测告警信息
    * @param [IN] lpUserData                          用户数据
    * @note
    */
    ...
    */
	 ...
	 * @param [IN] dwLogFileSize     单个日志文件大小(单位字节) The size of single log file //最大30M
	...
	/**
	 * 设置元数据绘图显示类型参数
	 * @param [IN] dwShowParam              元数据显示类型参数   参照结构体NETDEV_IVA_SHOW_RULE
	 * @return TRUE表示成功,其他表示失败
	 * @note
	 * -    1.该接口函数仅支持Windows.
	 ...
	 * 设置5ePTZ显示模式
	 * @param [IN] lpPlayHandle                  播放句柄
	 * @param [IN] udwPtzMode                    5ePTZ显示模式  参考# NETDEV_PTZ_DISPLAY_MODE_E
	 * @return TRUE表示成功,其他表示失败
	 * @note无
	 */
	...
	* @param [IN] lpUserID              用户登录ID User login ID
	* @param [IN] cbEventMessCallBack   回调函数 Callback function
	* @param [IN] lpUserData            用户数据 User data
	* @return TRUE表示成功，其他表示失败 TRUE means success, and any other value means failure.
	* @note
	*/
	...
	* 将指定人脸库同步至设备
	* @param [IN]  lpUserID             用户登录句柄 User login ID
	* @param [IN]  udwPersonLibID       人脸库ID Person Library ID
	* @param [IN]  pstDeviceIDList      设备列表 Device List
	* @return TRUE表示成功,其他表示失败
	* @note 无
	*/
	...
	*/
	 ...
	 * @param [IN]  lpUserID                    用户登录ID
	 * @param [IN]  udwID                       车辆布控ID
	 * @param [IN] pstMonitorInfo            布控任务信息
	 * @return TRUE表示成功，其他表示失败 TRUE means success, and any other value means failure.
	 * @note 车辆布控比对照片不超过2M,内存由使用者维护
	 */
	 ...
	 * 本地升级设备自身
	 * @param [IN]   lpUserID                用户登录ID User login ID
	 * @param [IN]   dwUpgradeType           升级类型，参考#NETDEV_UPGRADE_TYPE_E
	 ...
	 * 设备通道升级
	 * @param [IN]   lpUserID                用户登录ID User login ID
	 * @param [IN]   pstUpgradeInfo          需升级设备信息指针
	 * @param [INOUT]  pszTaskNo            设备返回的任务号,建议长度#NETDEV_LEN_64
	 * @return TRUE表示成功,其他表示失败
	 * @note无
	 */
	...
	* @param [IN]   pstPlayBackCond       回放参数 Play Back Condition
	* @param [OUT]  pszUrl                存储码流URL Record Stream Url
	* @return TRUE表示成功，其他表示失败
	*/
	...
	* 启动目标检索
	* @param [IN]   lpUserID                    用户登录句柄 User login ID
	* @param [IN]   pstStartInfo                查询开始信息 Query start information
	...
	* 获取目标检索查询进度
	* @param [IN]   lpUserID                    用户登录句柄 User login ID
	* @param [IN]   udwSearchID                 业务号 Search ID
	...
	* 查询目标检索结果
	* @param [IN]   lpUserID                    用户登录句柄 User login ID
	* @param [IN]   pstQueryCond                查询条件 Query Condition
	...
	*/
	...
	/**
	* 逐个获取查找到的目标检索结果
	* @param [IN]   lpFindHandle                    查找句柄 Query handle
	...
	*/
	...
	/**
	* 关闭目标检索结果查找,释放资源
	* @param [IN]   lpFindHandle                    图片查找句柄 Qearch handle
	* @return TRUE表示成功,其他表示失败
	* @note
	*/
	...
	* 获取指定告警的关联数据 获取查询句柄 
	* @param [IN]  lpUserID                          用户登录ID User login ID
    * @param [IN]  dwAlarmID                         告警 ID Alarm ID
	...
	* 导入音频文件
	* @param [IN]  lpUserID                        用户登录ID User login ID
	* @param [IN]  pszFilePath                     音频文件路径 导入音频须为PCM格式，文件大小不得超过100K
	* @return TRUE表示成功，其他表示失败
	* @note 
	*/
	...
	* 查询音频输出参数
	* @param [IN]  lpUserID                        用户登录ID User login ID
	* @param [IN]  udwChannelID                    通道ID Channel ID
	...
	* @param [IN]  lpUserID                        用户登录ID User login ID
	* @param [IN]  udwChannelID                    通道ID Channel ID
	* @param [IN]  pstAudioOutputCfg               音频输出参数 Audio OutPut Config
	* @return TRUE表示成功，其他表示失败
	* @note 
	*/
	...
	* 添加人员密度报警统计组信息
	* @param [IN] lpUserID                  用户登录ID
	* @param [IN] pstCrowdDensityGroupInfo  人员密度报警统计组信息
	* @param [OUT] pUdwGroupID              统计组ID
	* @return TRUE表示成功，其他表示失败 TRUE means success, and any other value means failure.
	*/
	...
	* 删除人员密度报警统计组信息
	* @param [IN] lpUserID                 用户登录ID
	* @param [IN] udwCrowdDensityGroupID   人员密度报警统计组id
	* @return 
	*/
	...
	*/
	...
	/**
	* 逐个获取查找到的人员密度报警统计组信息
	* @param [IN] lpFindHandle                              人员密度报警统计组列表查询句柄
	* @param [OUT] pstCrowdDensityGroupInfo                 门禁授权组列表信息
	* @return TRUE表示成功，其他表示失败
	*/
	...
	* 接收告警消息的回调函数
	* @param [IN] lpUserID              用户登录ID
	* @param [IN] pstReportInfo         上报信息
	* @param [IN] szReference           描述信息
	* @param [IN] lpUserData            用户数据 
	* @note  
	*/
	...
	* @param [OUT] pstSceneDetailInfo  场景信息结构体
	* @return TRUE表示成功，其他表示失败
	*/
	...
	* 电视墙开启回放(通过时间)
	* @param [IN] lpUserID                                          用户登录ID
	* @param [INOUT] pstPlayBackCond                                回放参数(不需要填写sessionID字段，开启成功后返回sessionID字段)
	* @return TRUE表示成功，其他表示失败
	*/
	...
	* 道路监控手动触发抓拍
	* @param [IN]   lpUserID                  用户登录ID User login ID
	* @param [IN]   pstManualSnapParam        手动抓拍参数
	* @return TRUE表示成功,其他表示失败
	* @note
	*/
	public boolean NETDEV_MaunalSnapshot(Pointer lpUserID, int dwChannelID, NETDEV_MANUAL_SNAPSHOT_PARAM_S pstManualSnapParam);	
}